...
const kubernetesApiHost =
  process.env.KUBERNETES_SERVICE_HOST || "kubernetes.default.svc";

const kubernetesApiPort =
  Number(process.env.KUBERNETES_SERVICE_PORT_HTTPS) || 443;
...
app.get("/api/backend/magic/:name", async (req, res) => {
  const { name } = req.params;

  try {
    const response = await fetch(
      `${backendUrl}/api/magic/${encodeURIComponent(name)}`,
      {
        cache: "no-store",
      }
    );
    ...
    console.error("Backend magic value error:", error);
...
app.post("/api/backend/magic/:name", async (req, res) => {
  const { name } = req.params;
  ...
  try {
    ...
    console.error("Backend magic value update error:", error);
